 ...
 * File:   main.cpp
 * Author: knguyen
 * Purpose: Adding and adjusting pseudocode based on requirements given
 */

/*
     * add:
     * function-
     * roll dice/damage. replace so rollvalue = roll function, add roll to vector,
     * if d of roll > 100 output not a valid die, exit()
     * rolls vector: vector, initialize to 0, selection sort and output rolls, search for value;
     * 
     * enemies 2d array, just different name/stats
     *      bubble sort by hp, save original index into order parallel array,
     *      output enemies using this order array, choose based on i and find order number to use to get value from array
*/
...
//prototypes
//function to roll dice, int roll(vector<int>& rolls, bool save = false, bool isPlayer = false, int roll number = 1, int sides = 20)
//function to selection sort vector, void void sort (vector<int>& rolls, int size)
//function to print vector void prntm (vector<int>)
//function to search vector, binary search, bool search (vector<int> rolls, int size, int val, int& index)
...
//int roll(vector<int>& rolls, bool save = false, bool isPlayer = false, int roll number = 1, int sides = 20)
/*
 * int one roll
 * if sides = 0
 *      output error_0
