 ...
 * File:   main.cpp
 * Author: knguyen
 * Purpose: Writing enemies arrays, and sort and print function, debugging reordering by copying array.
 */
...
static const int ESTATS = 5,
        ESIZE = 3;
    
//prototypes
int roll(vector<int>& rolls, bool save = false, bool isP = false, int n = 1, int d = 20); //function to roll dice
void selSrt(vector<int>&); //function to selection sort vector
void prntm (vector<int>); //function to print vector
bool binSrch (vector<int>, int, int&); //function to search vector, binary search
void bubSrt (int [][ESTATS], int[]); //function to bubble sort
void prntm (string [], int[]); //function to print array
...
int main(int argc, char** argv)
{
    ...
    char cenemy; //store enemy choice as char
    int enemy; //enemy choice as int
    string eaname[ESIZE] = {"ogre", "dragon", "kobold"}; //enemy names
    int evals[ESIZE][ESTATS] = //enemy stats
    {
        {70, 15, 6, 2, 6},
        {130, 17, 8, 3, 8},
        {10, 14, 4, 1, 8}
    };
    int order[ESIZE]; //array to store new order
    int eind; //set order input
    //sort and print
    bubSrt(evals, order); //sort in order
    prntm(eaname, order); //print in order, starting at 1
    
    //choose enemy
    do
    {
        cout << endl << "Choose an enemy, from 1 to " << ESIZE << endl; //prompt input
        cin >> cenemy; //store input
        enemy = cenemy - '0'; //convert input to integer
        if (enemy < 1 && enemy > ESIZE) //if invalid
        {
            cout << endl << "Invalid entry. Reenter." << endl; //output invalid message
        }
    } while (enemy < 1 && enemy > ESIZE); //do while validate
    
    //set enemy stats
    eind = order[enemy - 1]; //index equals enemy choice - 1, in spot saved by new order array
    string ename = eaname[eind]; //enemy name
    int ehp = evals[eind][0], //health points
            eac = evals[eind][1], //armor class
            ebns = evals[eind][2], //attack bonus
            en = evals[eind][3], //number of dice
            ed = evals[eind][4]; //dice sides
    ...
    while (isD == false && iseD == false) //while both are alive
    {
        ...
        //enemy attack
        //output attack
        cout << endl << "The " << ename << " lunges at you." << endl;
        er20 = roll(rolls, false, false); //roll for attack
        erdmg = roll(rolls, false, false, en, ed); //roll for damage
        ...
        if (clas == '0')
        {
            ...
            if (act >= '1' && act <= '4') //if attack action
            {
                ...
                //enter for "roll"
                cin >> yn;
                r20 = roll(rolls, true, true); //roll d20, save roll, output
                ...
                //enter for "roll"
                cin >> yn;
                dmg = roll(rolls, false, true, n, d); //roll for damage
        ...
        if (clas == '1')
        {
           ...
           //execute actions with rolls
            ...
            else if (act == '3') //else if 3 you cast heal, roll to heal, adjust hp
            {
                ...
                //enter for "roll"
                cin >> yn;
                dmg = roll(rolls, false, true, n, d); //roll for healing points
                dmg += bns; //total roll for healing + bonus
                cout << "Spell bonus: " << bns << ". "; //output bonus
    ...
    //replay screen
    int val, //to store value
            line; //to store line found
    do
    {
        //prompt input
        cout << endl << endl << "Press 1 for replay, press 0 to exit." << endl;
        //store input
        cin >> yn;
        if (yn != '1' && yn != '0') //if invalid
        {
            //output invalid message
    ...
    if (yn == '1') //if replay
    {
        selSrt(rolls); //selection sort rolls
        prntm(rolls); //print rolls in order from least to greatest
        do
        {
            //prompt input
            cout << endl << "Press 1 to search for a roll, press 0 to exit." << endl;
            //store input
            cin >> yn;
            if (yn == '1') //if searching
            {
                 cout << endl << "Input a value to search for." << endl; //prompt input
                 cin >> val; //store input
                 if( binSrch(rolls, val, line) == true ) //if found
                 {
                    cout << endl << val << " was found around line " << line << endl; //output result
                 }
                 else //if not found
                 {
                    cout << endl << val << " was not rolled." << endl; //output result
                 }
            }
        } while (yn == '1'); //while searching
    }
    //exit
    return 0;
}

int roll(vector<int>& rolls, bool save, bool isP, int n, int d)
{
    int roll1 = 0; //single roll
    int tot = 0; //total roll
    if (d == 0) //if sides is 0
    {
        cout << endl << "error_0" << endl; //output error, unable to divide by 0
        exit(0); //exit
    }
    for (int i = 1; i <= n; i++) //roll for damage for loop number of dice
    {
        roll1 = rand()%d +1; //damage roll
        tot += roll1; //add to total damage
        if (isP == true) //if player
        {
            cout << "You rolled a " << roll1 << ". "; //output roll
            if (save == true) //if saving is on
            {
                rolls.push_back(tot); //add to vector
            }
        }
    }
    cout << endl; //final endl
    return tot; //return total roll
}
...
void selSrt(vector<int>& vec)
{
    ...
    for (int i = 0; i < vec.size(); i++) //loop each i until end of vector
    {
        ...
        {
            if (vec[i] > vec[j]) //if greater than next
            {
...
bool binSrch(vector<int> vec, int val, int& index)
{
    ...
    do
    {
        ...
        else if (vec[mind] > val) //if greater
        {
            hind = mind - 1; //mid is new low
        }
    } while (lind <= hind); //until search finished
    return false; //return bool if found
}

void bubSrt (int values[][ESTATS], int order[])
{
    int copy[ESIZE]; //array to copy
    //copy array
    for (int i = 0; i < ESIZE; i++)
    {
        copy[i] = values[i][0]; //set to each first num in value array
    }
    ...
    int n = ESIZE-1; //n to loop to
    do
    {
        cont = false; //dont continue if no swaps
        for (int i = 0; i < n; i++) //loop each i
        {
            if (values[i][0] > values[i+1][0]) //if greater than next
            {
                //sort copy
                temp = copy[i]; //save first
                copy[i] = copy[i+1]; //switch first and nest, new a[i] to compare to next j until ai is the smallest
                copy[i+1] = temp; //set next to first
                //update order
        ...
        n--; //last is largest
    } while (cont == true); //while still swapping
}

void prntm (string names[], int order[])
{
    cout << endl << endl; //new line
    for (int i = 0; i < ESIZE; i++) //loop through array in new order
    {
        cout << (i+1) << ": " << names[order[i]] << endl; //output array
    }
}